//Start
const herukoStart = `READY TO PLAY`
...
(async () => {
    ...
    try {
        ...
        login({ appState }, async (error, api) => {
            ...
            const Commands = readdirSync(join(__dirname, 'modules', 'commands')).filter(command => command.endsWith('.js') && !command.includes('example') && !global.config.commandDisabled.includes(command));
            for (const command of Commands) {
                try {
                    var module = require(join(__dirname, 'modules', 'commands', command));
            ...
            const events = readdirSync(join(__dirname, 'modules', 'events')).filter(event => event.endsWith('.js') && !global.config.eventDisabled.includes(event));
            for (const event of events) {
                try {
                    var module = require(join(__dirname, 'modules', 'events', event));
